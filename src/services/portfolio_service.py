    ...
    def _replay_account(self, *, account: Any, as_of_date: date, cost_method: str) -> Dict[str, Any]:
        ...
        for event_type, event_date, _, event in events:
            ...
            if event_type == "trade":
                ...
                # Screenshot-imported legacy positions can legitimately carry a
                # negative effective cost basis after prior profit recycling.
                # Keep manual write validation strict elsewhere, but allow
                # replaying already-recorded non-zero prices here so the web
                # portfolio can render those holdings.
                if abs(price) <= EPS:
                    raise ValueError(f"Invalid trade price for {event.symbol}")
