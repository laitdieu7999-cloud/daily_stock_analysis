...
# 针对特别吵的第三方子 logger 做更细粒度的级别覆盖。
# 例如 urllib3.connectionpool 的 Retry 警告会在 Eastmoney/efinance
# 不稳定时刷满主日志，但真正的业务失败日志仍由仓库自己的 fetcher
# 汇总输出，因此这里压到 ERROR，避免重复噪音。
DEFAULT_LOGGER_LEVEL_OVERRIDES = {
...
def setup_logging(
    ...
    for logger_name, logger_level in DEFAULT_LOGGER_LEVEL_OVERRIDES.items():
