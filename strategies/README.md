...
当你维护的是“黄金 / 白银 / 股指期货 / 星象辅助因子”这类长期会持续扩展的策略时，建议采用：

- 一个总入口策略：负责定义全局边界、方向约束、跨资产协作关系
- 多个子策略文件：每个文件只负责一个资产或一个研究主题

例如：

```text
asset_direction_policy.yaml          # 总入口 / 全局约束
gold_long_accumulation.yaml          # 黄金子策略
silver_dual_direction_framework.yaml # 白银子策略
ic_basis_roll_framework.yaml         # IC子策略
```

这样做的好处是：

- 不同资产的规则不会持续堆在一个文件里
- 你后续加入星象、天干地支等辅助模块时，可以独立试验，不容易污染主策略
- 单个策略文件更容易调试、回顾和逐步产品化
