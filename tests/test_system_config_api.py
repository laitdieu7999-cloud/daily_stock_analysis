...
class SystemConfigApiTestCase(unittest.TestCase):
    ...
    def test_get_config_accepts_extended_registry_categories_in_schema(self) -> None:
        payload = system_config.get_system_config(include_schema=True, service=self.service).model_dump(by_alias=True)
        item_map = {item["key"]: item for item in payload["items"]}

        self.assertEqual(item_map["EXTERNAL_TACTICAL_REPORT_PATH"]["schema"]["category"], "strategy")
        self.assertEqual(item_map["CLOSE_REMINDER_TIME"]["schema"]["category"], "schedule")
        self.assertEqual(item_map["JIN10_API_KEY"]["schema"]["category"], "integration")
        self.assertEqual(item_map["ENABLE_METAPHYSICAL_FEATURES"]["schema"]["category"], "advanced")
